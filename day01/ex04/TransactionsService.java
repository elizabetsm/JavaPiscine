...
public class TransactionsService {
    ...
    public void makeTransactions(Integer senderId, Integer recipientId, Integer amount) {
        ...
        if (amount > 0 && sender.getBalance() < amount || amount < 0 && recipient.getBalance() < -amount) {
            throw new IllegalTransactionException("Transaction is illegal. Try another one");
        }

        if (amount > 0) {
            Transaction transaction1 = Transaction.createTransaction(recipient, sender, TransactionType.DEBIT, amount);
            Transaction transaction2 = Transaction.createTransaction(sender, recipient, TransactionType.CREDIT, -amount);
            ...
            Transaction transaction1 = Transaction.createTransaction(sender, recipient, TransactionType.DEBIT, -amount);
            Transaction transaction2 = Transaction.createTransaction(recipient, sender, TransactionType.CREDIT, amount);
            transaction2.setIdentifier(transaction1.getIdentifier());
